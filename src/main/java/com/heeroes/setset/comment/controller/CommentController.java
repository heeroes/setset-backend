...
public class CommentController {
    ...
    @GetMapping("/{groupId}/article/{articleId}/comment")
    public ResponseEntity<Response<List<CommentResponse>>> getAllByArticleId(@PathVariable int groupId,
                                                                             @PathVariable int articleId,
                                                                             @RequestHeader("Authorization") String tokenHeader) {
        int userId = jwtTokenProvider.extractUserId(tokenHeader.substring(7));
        List<CommentResponse> commentResponses = commentService.getAllByArticleId(articleId, userId, groupId);
        return ResponseEntity.ok(Response.success(commentResponses));
    }
}
